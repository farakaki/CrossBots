...
def blackjack():
    ...
    def escolher_carta():
        carta_naipe = naipe_cartas[random.randint(0, 3)]
        numero_valor = random.randint(0, 12)
        ...
        if numero_valor >= 9:  
            ...
            valor = numero_valor + 1  

        return [carta_escolhida, valor]

    def carta_igual(carta, cartas_usadas):
    ...
    def imprimir_cartas(cartas_dealer, cartas_jogador):
        print("\nCartas Jogador: ")
...
def roleta():
    ...
    lucro = 0
    teste = False
    num_vermelho = [1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36]
    ...
    while True:
        ...
        if apostar == 's':
            if saldo >= 100:
                ...
                for i in range(3):
                    simbolo_escolhido.append(random.choice(lista_simbolo))
                    time.sleep(3)
